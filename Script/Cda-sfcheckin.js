...
!(async () => {
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    // ========================================================
    // ✨【安全重置】仅仅在循环开始时，把当前账号的局部快照变量清空，不干扰请求头
    // ========================================================
    $.login = null
    $.sign = null
    $.tasks = []
    ...
    try {
      ...
      await loginApp(acc)
      await $.wait(2000) // 稍微拉长等待，让网络层稳定
      await loginWeb()
      await $.wait(2000)
      await sign()
      await $.wait(2000)
      await signDailyTasks()
      results.push(formatMsg(acc))
    } catch (e) {
      console.log(`❌ 账号 ${$.currentMobile} 出错:`, e)
      results.push(`账号 ${$.currentMobile} 出错: ${e}`)
    }
    
    // 账号之间等 3 秒，防止底层 Cookie 并发错乱
    if (i < accounts.length - 1) {
      console.log(`等待 3 秒后执行下一个账号...`)
      await $.wait(3000)
    }
  }
...
function loginApp(account) {
  return $.http.post(account).then((resp) => {
