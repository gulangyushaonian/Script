  ...
  if (!rsp) {
    ...
    try {
      ...
      for (const images of cache_body) {
        if (images && images.live_photo_file_id && images.live_photo?.media) {
          const media = images.live_photo.media;
          // 优先取 h265 的 master_url，备选 h264 或 url
          const stream = media.stream?.h265?.[0] || media.stream?.h264?.[0];
          const rawUrl = stream?.master_url || stream?.url || "";
      ...
      // 如果官方返回体里包含 datas 数组，进行精确覆盖与错误码擦除
      if (obj?.data?.datas && Array.isArray(obj.data.datas)) {
        obj.data.datas.forEach(itemA => {
          const itemB = new_data.find(b => b.file_id === itemA.file_id);
          if (itemB) {
            itemA.url = itemB.url;
            itemA.author = "@fmz200";
            // 关键擦除：清除官方的报错标识，强制让 APP 识别为成功
            if (itemA.hasOwnProperty("error_code")) {
              delete itemA.error_code;
            }
            if (itemA.hasOwnProperty("error_msg")) {
              delete itemA.error_msg;
            }
          }
        });
      } else {
      ...
      console.log('处理 live_photo 数据发生错误: ' + e);
    }
  }
}
